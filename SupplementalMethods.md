...
v1.14.6](https://github.com/tseemann/prokka/releases/tag/v1.14.6) with
default parameters. Prokka annotations were imported into Anvi’o
following the recommended [Anvi’o pipeline for Prokka annotated
...
files; one containing the gene calls data and another with the
...
COGs database 2020 release. We used the DIAMOND “sensitive” option
instead of the default “fast” one.
...
The program
[`anvi-run-pfams`](https://merenlab.org/software/anvio/help/main/programs/anvi-run-pfams/)
annotates the .db genomes with the PFAM database.
...
stores hmm-hits into the .db genomes files.
...
command in `analysis_Anvio7/AnvioGenomesSummaryFuntions.txt` includes
details on all the functional annotations included in the .db files.
...
We use the program `anvi-pan-genome` to run the actual pangenomic
analysis. We used the more sensitive blastp search (`--use-ncbi-blast`)
instead of the default DIAMOND fast option. `--mcl-inflation` was set to
10, as recommended for very closely related genomes (i.e., ‘strains’ of
the same ‘species’).
