...
TableGene$Percentage <- round(100*(TableGene$Count/49415),1)
...
The total sum of all values in the `num_corrected_genes` variable should add up to the number of GCs:
```{r, message=FALSE}
...
We search for individual genes annotated as Transposases/Retrons in either the COG or Prokka annotations:
```{r}
...
```
There are `r nrow(Transposase_Genes)` individual genes annotated in either the COG or Prokka annotations as Transposases and `r nrow(Retron_Genes)` as Retrons. However, they belong to GCs that contain some extra individual genes that lack annotation, but should be considered also, since they belong to the same cluster.

The following datasets include all the members of the GCs identified in the previous step:
```{r}
...
```
Therefore, taking into account all the members of the selected GCs there are `r nrow(Transposase_Genes_FullGCs)` individual genes identified as Transposases and `r nrow(Retron_Genes_FullGCs)` as Retrons.

### Transposase Tables

Summary matrix table with a Genome in each row and each transposase GC listed in each column.
```{r message=FALSE}
...
We focus on the main retron, GC number GC_00000001:
```{r message=FALSE}
