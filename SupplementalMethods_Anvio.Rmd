...
The `anvi-run-ncbi-cogs` command was used to annotate the .db genome files against the NCBI's COGs database 2020 release [@tatusov1997; @galperin2020]. We used the DIAMOND [@buchfink2014] "sensitive" option instead of the default "fast" one.
...
The program `anvi-run-kegg-kofams` annotates the .db genomes with KEGG Orthology (KO) numbers via hits to the KEGG KOfam database [@Kanehisa2000; @Kanehisa2016].
